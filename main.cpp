...
/*void draw(WINDOW* winTrackSel, WINDOW* winTrack, WINDOW* winSound, std::vector<Track*> tracks, Track* currentTrack, int selectedOption) {
  refresh();
  box(winTrackSel, 0, 0);
  box(winTrack, 0, 0);
  box(winSound, 0, 0);
  ...
  mvwprintw(winTrackSel, 0, (auxx / 2) / 2 - std::string(" Select Track ").size() / 2, " Select Track ");
  mvwprintw(winSound, 0, (auxx / 2) / 2 - std::string(" Sound Visualizer ").size() / 2, " Sound Visualizer ");
  ...
  for (int i = 0; i < tracks.size(); i++) {
    ...
    s = line.size();
    for (int j = 0; j < (auxx / 2) - s - 2; j++) line += " ";
